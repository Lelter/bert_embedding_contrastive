...
class MaskCTR(nn.Module):
    def __init__(
        ...
        self.rec_encoder = RecEncoder_DCNv2(struct_linear_feature_columns, struct_dnn_feature_columns,
                                            struct_feature_num)
        ...
        self.config = {'num_fields': self.struct_field_size, 'proj_size': 32, 'input_size': struct_feature_num,
    ...
    def forward(self, batch):
        ...
        if self.use_mlm:
            mask_text_features = self.text_encoder.mlm_forward(input_ids=batch["mask_input_ids"],
                                                               attention_mask=batch["attention_mask"])  # B N D
            mask_text_labels = batch['mask_text_label']

            text_pred_in = torch.cat(
                [mask_text_features, rec_features.unsqueeze(1).repeat((1, mask_text_features.shape[1], 1))],
                dim=-1)  # B N D+H
            text_pred_out = self.text_pred_module(text_pred_in)
            mlm_loss = nn.CrossEntropyLoss()(text_pred_out.view(-1, self.text_tokenizer_num),
                                             mask_text_labels.long().view(-1))
            # print(mlm_loss)
