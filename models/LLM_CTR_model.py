...
from layers.modules import TextEncoder, RecEncoder_DeepFM, RecEncoder_PNN, RecEncoder_DCNv2, RecEncoder_AutoInt, \
    RecEncoder_xDeepFM, RecEncoder_DCN, RecEncoder
...
class bertCTRModel(nn.Module):
    def __init__(
        ...
        # self.rec_projection = ProjectionHead(rec_embedding_dim, projection_dim, dropout=cfg.dropout)
        # self.text_projection = ProjectionHead(projection_dim*6, projection_dim*6, dropout=cfg.dropout)
        # self.cross_attention = CrossAttention(projection_dim, projection_dim, projection_dim, projection_dim, 8)
        # self.final_predictor = nn.Linear(projection_dim, 1)
        # self.final_out = PredictionLayer()
        if cfg.llm == "SFR":
    ...
    def domain_inner_loss(self, text_features_list1, text_features_list2):
        ...
        for i in range(num_domains):
            ...
            neg_exp = torch.exp(sim_neg / self.t).sum() - torch.exp(torch.diag(sim_neg) / self.t).sum()#除了对角线，其他的都是负样本
    ...
    def domain_in_cross_regulation_loss(self, text_features_list1):
        ...
        # 计算softmax后的相似度矩阵
        sd_matrix = sd_matrix / sd_matrix.norm(p=2)
    ...
    def forward(self, batch,mode="train"):
        ...
        if mode == "train":
            ...
            total_loss=[domain_inner_loss,domain_in_cross_regulation_loss]
        else:
            # Compute logits
            logits = self.rec_encoder(text_features_list1, batch['rec_data'])
            total_loss = []
