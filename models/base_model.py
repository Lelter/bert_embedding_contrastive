...
class Linear(nn.Module):
    ...
    def forward(self,text_embedding_list,X=None, sparse_feat_refine_weight=None):
        sparse_embedding_list = [self.embedding_dict['all'](
            X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]].long())
            for feat in self.sparse_feature_columns]

        linear_logit = torch.zeros([text_embedding_list[0].shape[0], 1]).to(text_embedding_list[0].device)
        if len(text_embedding_list) > 0:
            text_embedding_cat = torch.cat(text_embedding_list, dim=-1)#线性层输入有误
            sparse_embedding_cat=torch.cat(sparse_embedding_list,dim=-1)
            sparse_embedding_cat=sparse_embedding_cat.squeeze(1)
            all_embedding_cat=torch.cat([text_embedding_cat,sparse_embedding_cat],dim=-1)
            if sparse_feat_refine_weight:
                all_embedding_cat = all_embedding_cat * sparse_feat_refine_weight.unsqueeze(1)#加权
            sparse_feat_logit = torch.sum(all_embedding_cat, dim=-1, keepdim=False)
            linear_logit += sparse_feat_logit.unsqueeze(1)
        # if len(dense_value_list) > 0:
        #     dense_embedding_logit = torch.cat(dense_value_list, dim=-1).matmul(self.weight)
        #     linear_logit += dense_embedding_logit

        return self.BN(linear_logit)


class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, total_feature_num, l2_reg_linear=1e-5,
