...
def data_process(data_path, sparse_features, data_source):
    data = pd.read_csv(data_path, encoding='utf-8')
    data['label'] = data['rating'].apply(lambda x: 1 if x > 3 else 0)
    # 丢弃为3的
    # data = data[data['rating'] != 3]  # 提AUC
    for feat in sparse_features:
        lbe = LabelEncoder()
        data[feat] = lbe.fit_transform(data[feat])
    # 按照时间排序
    # data = data.sort_values('timestamp')  # 提AUC
...
if __name__ == '__main__':
    ...
    if data_source == 'movielens':
        ...
        # sparse_features = ['user_id', 'movie_id', 'gender', 'age', 'occupation', 'zip', 'genres', 'title']
        sparse_features = ['gender', 'age', 'occupation', 'zip', 'genres', 'title']
        target = ['label']
    ...
    train, test, data = data_process(data_path, sparse_features, data_source)
    ...
    models = ['DeepFM', 'AutoInt', 'DCN', 'WideDeep', 'DCNv2', 'xDeepFM', 'PNN', 'AFN', 'AFM']
    ...
    for model_name in models:
        ...
        if model_name == 'DeepFM':
            model = DeepFM(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                           dnn_dropout=dropout,
                           seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'AutoInt':
            model = AutoInt(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                            dnn_dropout=dropout,
                            seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'DCN':
            model = DCN(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                        dnn_dropout=dropout,
                        seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'WideDeep':
            model = WideDeep(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                             dnn_dropout=dropout,
                             seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'DCNv2':
            model = DCNMix(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                           dnn_dropout=dropout,
                           seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'xDeepFM':
            model = xDeepFM(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                            dnn_dropout=dropout,
                            seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'PNN':
            model = PNN(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                        dnn_dropout=dropout,
                        seed=seed, dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'AFN':
            model = AFN(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                        dnn_dropout=dropout,
                        seed=seed, afn_dnn_hidden_units=(300, 300, 128), device=device)
        elif model_name == 'AFM':
            model = AFM(linear_feature_columns, dnn_feature_columns, total_feature_num, task='binary',
                        seed=seed, device=device)

        model.compile("adagrad", "binary_crossentropy", metrics=['auc', 'accuracy', 'logloss']
                      , lr=lr)

        test_model_input = {name: test[name] for name in sparse_features}

        model.fit(train_model_input, train[target].values, batch_size=batch_size, epochs=epoch, verbose=0,
        ...
        print(f"Model: {model_name}, Test LogLoss: {logloss}, Test AUC: {auc}")
        writer_text = [model_name, str(epoch), str(batch_size), str(lr), str(dropout), auc, logloss]
