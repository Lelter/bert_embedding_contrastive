...
from config_pretrain import create_parser
from datasets import BertCTRDataset
from models.LLM_CTR_pretrain_model import bertCTRModel
...
def process_struct_data(data_source, train, val, test, data):
    ...
    sparse_features = {
        ...
        'bookcrossing': ['userId', 'ISBN','bookTitle', 'bookAuthor', 'yearOfPublication', 'location', 'publisher', 'age' ],
...
def main(cfg):
    ...
    model = bertCTRModel(cfg, text_encoder_model=cfg.text_encoder_model,)
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_idx, (batch) in pbar:
            ...
            pbar.set_description(
                f"epoch {epoch + 1} : constraint loss2 {loss.item():.5f}"
            )
            train_loss_list.append(loss)
        if accelerator.is_local_main_process:
            print(sum(train_loss_list) / len(train_loss_list))
        numpy_list = [tensor.cpu().detach().numpy() for tensor in train_loss_list]
        numpy_array = np.array(numpy_list)
        plt.plot(numpy_array)
        # plt.show()
    torch.save(model.state_dict(), f'ckpts/model_{cfg.dataset}_{cfg.llm}_no_cross.pth')
