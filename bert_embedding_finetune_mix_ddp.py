...
from config_mix_finetune import create_parser
from datasets import BertCTRDataset
from models.LLM_CTR_finetune_mix_model import bertCTRModel
...
def main(cfg):
    ...
    model.load_state_dict(torch.load(f'ckpts/model_{cfg.dataset}_{cfg.llm}_modal_lower.pth',map_location={'cuda:4':'cuda:1'}), strict=False)
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_idx, (batch) in pbar:
            ...
            rec_pred, text_pred, y_pred = model(batch)
            loss_fun=BCEWithLogitsLoss()
            rec_loss = loss_fun(rec_pred.squeeze(1), label)
            text_loss = loss_fun(text_pred.squeeze(1), label)
            y_loss = loss_fun(y_pred.squeeze(1), label)
            # loss = y_loss
            loss = rec_loss+text_loss+y_loss
            # focalLoss = FocalLoss()
            ...
            pbar.set_description(
                f"epoch {epoch + 1} : train loss {loss.item():.5f},lr1:{optimizer.param_groups[0]['lr']:.5f},lr2:{optimizer.param_groups[1]['lr']:.5f}"
            )
...
def validate(val_loader, model):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            pres_list.append(torch.sigmoid(y_pred).squeeze(1).cpu().numpy())
