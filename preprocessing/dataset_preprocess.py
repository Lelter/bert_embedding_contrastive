...
def load_data():
    ...
    df.to_csv('./data/ml-1m/struct_data_no_time.csv', index=False)
    ...
    df['result'] = result
    df['result'].to_csv('./data/ml-1m/text_no_time.txt', index=False)
...
def text_ml_csv():
    ...
    # 打印后五行
    print(df.tail())
    df.to_csv('./data/ml-1m/text_data.csv', index=False)
    return df


def text_amazon_csv():
    # 读取数据
    data_path = './data/amazon/struct2.csv'
    df = pd.read_csv(data_path)
    ...
    # 删去user_id、movie_id、rating、timestamp和label列
    df = df.drop(columns=['user_id','item_id','label'])
    print(df.head())
    print(df.info())
    # 打印后五行
    print(df.tail())
    df.to_csv('./data/amazon/text_data.csv', index=False)
    return df
...
def tokenize_batch(batch, tokenizer):
    ...
    prompt = (
        f"Transform the sentence from subject-verb-object structure by substituting the subject with various synonyms and rewriting the sentence in different structures while maintaining the object unchanged. You may include additional words.Only output one sentence.")
...
def data_enhancement():
    ...
    sampling_params = SamplingParams(temperature=0.7, max_tokens=50, min_p=0.01,
    ...
    df.to_csv('./data/ml-1m/rephrase_data2.csv', index=False)


def amazon_cal_maxlens():
    tokenizer = AutoTokenizer.from_pretrained("./pretrained_models/distilbert-base-uncased", local_files_only=True)
    data = pd.read_csv('./data/amazon/text_data.csv')
    token_counts = data.applymap(lambda x: len(tokenizer.tokenize(x)))

    # 计算每列中的最大token数量
    max_token_counts = token_counts.max()

    # 打印每列的最大token数量
    print(max_token_counts)
