...
from models.base_mix_model import BaseModel, Linear
...
class RecEncoder_mix_WideDeep(BaseModel):
    def __init__(self, cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
        ...
        super(RecEncoder_mix_WideDeep, self).__init__(cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
        ...
        self.use_dnn = len(sparse_feature_columns) > 0 and len(dnn_hidden_units) > 0
        # self.alpha = nn.Parameter(torch.tensor(1., device=device), requires_grad=True)
        # self.text_out = PredictionLayer()
        # self.rec_out = PredictionLayer()
        # self.out = PredictionLayer()
        self.out=None
        self.text_dense = nn.Linear((len(sparse_all_feature_columns) - len(sparse_feature_columns)) * 32, 1)
        if self.use_dnn:
            self.dnn = DNN(self.compute_input_dim(sparse_feature_columns), dnn_hidden_units,
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        ctr_logit = self.linear_model(rec_inputs)
        text_logit = self.text_dense(torch.cat(text_embedding_list, dim=1))

        if self.use_dnn:
            dnn_sparse_input = combined_dnn_input(sparse_embedding_list, dense_value_list)
            dnn_input = dnn_sparse_input
            # dnn_input = combined_dnn_input(sparse_embedding_list, dense_value_list)
            dnn_output = self.dnn(dnn_input)
            dnn_logit = self.dnn_linear(dnn_output)
            ctr_logit += dnn_logit

        ctr_logit = ctr_logit * self.alpha
        text_logit = text_logit * (1.0 - self.alpha)
        # y_pred = self.out(ctr_logit + text_logit)
        # text_pred = self.text_out(text_logit)
        # rec_pred = self.rec_out(ctr_logit)
        y_pred = ctr_logit + text_logit
        text_pred = text_logit
        rec_pred = ctr_logit
        # y_pred = self.out(logit / logit.norm(p=2))
        return rec_pred, text_pred, y_pred


class RecEncoder_mix_DeepFM(BaseModel):
    def __init__(self, cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num, use_fm=True,
        ...
        super(RecEncoder_mix_DeepFM, self).__init__(cfg,sparse_feature_columns, sparse_all_feature_columns,
                                                    total_feature_num,
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        sparse_embedding_list = sparse_embedding_list[:rec_inputs.shape[1]]
        ctr_logit = self.linear_model(X=rec_inputs)
        text_logit = self.text_dense(torch.cat(text_embedding_list, dim=1))
        if self.use_fm:
            fm_input = torch.cat(sparse_embedding_list,dim=1)
            ctr_logit += self.fm(fm_input)
...
class RecEncoder_mix_DCNv2(BaseModel):
    def __init__(self, cfg,sparse_feature_columns,
        ...
        super(RecEncoder_mix_DCNv2, self).__init__(cfg=cfg,linear_feature_columns=sparse_feature_columns,
                                                   dnn_feature_columns=sparse_all_feature_columns,
                                                   total_feature_num=total_feature_num,
        ...
        if len(self.dnn_hidden_units) > 0 and self.cross_num > 0:
            dnn_linear_in_feature = self.compute_input_dim(sparse_feature_columns) + dnn_hidden_units[-1]
        elif len(self.dnn_hidden_units) > 0:
            dnn_linear_in_feature = dnn_hidden_units[-1]
        elif self.cross_num > 0:
            dnn_linear_in_feature = self.compute_input_dim(sparse_feature_columns)

        self.dnn_linear = nn.Linear(dnn_linear_in_feature, 1).to(
            device)
        self.crossnet = CrossNetMix(in_features=self.compute_input_dim(sparse_feature_columns),
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        text_logit = self.text_dense(text_input)
        ...
        if self.dnn_hidden_units and self.cross_num:
            ...
            ctr_logit = torch.add(ctr_logit, self.dnn_linear(stacked_output))
            if self.cfg.mixed_precision:
                ctr_logit = ctr_logit * self.alpha
                text_logit = text_logit * (1.0 - self.alpha)
                y_logit=ctr_logit + text_logit
                return ctr_logit, text_logit,y_logit
            else:
                ctr_logit = ctr_logit * self.alpha
                text_logit = text_logit * (1.0 - self.alpha)
                # print(self.alpha)
...
class RecEncoder_mix_PNN(BaseModel):
    def __init__(self, cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
                 dnn_hidden_units=(300, 300, 128),
        ...
        super(RecEncoder_mix_PNN, self).__init__(cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
        ...
        product_out_dim = 0
        num_inputs = self.compute_input_dim(sparse_feature_columns, include_dense=False, feature_group=True)
        ...
        self.dnn = DNN(product_out_dim + self.compute_input_dim(sparse_feature_columns), dnn_hidden_units,
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        linear_signal = linear_sparse_signal

        if self.use_inner:
            inner_product_input = sparse_embeddings
        ...
        dnn_output = self.dnn(dnn_input)
        ctr_logit = self.dnn_linear(dnn_output)
        ctr_logit = ctr_logit * self.alpha
        text_logit = text_logit * (1.0 - self.alpha)
        y_pred = ctr_logit + text_logit
        text_pred = text_logit
...
class RecEncoder_mix_AutoInt(BaseModel):
    def __init__(self,cfg, sparse_feature_columns, sparse_all_feature_columns, total_feature_num, att_layer_num=3,
        ...
        super(RecEncoder_mix_AutoInt, self).__init__(cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
                                                 l2_reg_linear=0,
                                                 l2_reg_embedding=l2_reg_embedding, init_std=init_std, seed=seed,
                                                 task=task,
                                                 device=device, gpus=gpus)
        ...
        field_num = len(sparse_feature_columns)
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        if len(self.dnn_hidden_units) > 0 and self.att_layer_num > 0:  # Deep & Interacting Layer
            ...
            ctr_logit += self.dnn_linear(stack_out)
        elif len(self.dnn_hidden_units) > 0:  # Only Deep
            deep_out = self.dnn(dnn_input)
            ctr_logit += self.dnn_linear(deep_out)
        elif self.att_layer_num > 0:  # Only Interacting Layer
            ctr_logit += self.dnn_linear(att_output)
        else:  # Error
            pass
...
class RecEncoder_mix_xDeepFM(BaseModel):
    def __init__(self,cfg, sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
        ...
        super(RecEncoder_mix_xDeepFM, self).__init__(cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
                                                 l2_reg_linear=l2_reg_linear,
                                                 l2_reg_embedding=l2_reg_embedding, init_std=init_std, seed=seed,
                                                 task=task,
                                                 device=device, gpus=gpus)
        ...
        self.use_cin = len(self.cin_layer_size) > 0 and len(sparse_feature_columns) > 0
        if self.use_cin:
            # field_num = len(self.embedding_dict)
            field_num = len(sparse_feature_columns)
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        sparse_embedding_list, dense_value_list = self.input_from_feature_columns(rec_inputs,
                                                                                  ...
                                                                                  self.embedding_dict)
        sparse_embedding_list = sparse_embedding_list[:rec_inputs.shape[1]]
        linear_logit = self.linear_model(rec_inputs)
        text_input = torch.cat(text_embedding_list, dim=1)
        text_logit = self.text_dense(text_input)
        if self.use_cin:
...
class RecEncoder_mix_DCN(BaseModel):
    def __init__(self, cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num, cross_num=2,
                 ...
                 task='binary', device='cpu', gpus=None):
        super(RecEncoder_mix_DCN, self).__init__(cfg,sparse_feature_columns, sparse_all_feature_columns, total_feature_num,
                                             l2_reg_linear=l2_reg_linear,
        ...
        # Cross
        self.crossnet = CrossNet(in_features=self.compute_input_dim(sparse_feature_columns),
    ...
    def forward(self, text_embedding_list, rec_inputs):
        ...
        if len(self.dnn_hidden_units) > 0 and self.cross_num > 0:
            ...
            cross_out = self.crossnet(dnn_input)
            ctr_logit += self.dnn_linear(cross_out)
        else:
            pass
        ctr_logit = ctr_logit * self.alpha
        text_logit = text_logit * (1.0 - self.alpha)
